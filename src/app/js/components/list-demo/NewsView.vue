<template>
  <div class="news-view" :class="{ loading: !items.length }">
    ...
    <div class="nav" v-show="items.length > 0">
      <a v-if="page > 1" :href="'#/home/list-demo/' + (page - 1)">&lt; prev</a>
      <a v-if="page < 4" :href="'#/home/list-demo/' + (page + 1)">more...</a>
    </div>
  </div>
</template>
...
export default {
  ...
  route: {
    data ({ to }) {
      ...
      return store.actions.listDemo.fetchItemsByPage(page).then(items => ({
        page,
        items
      }))
    }
  ...
  created () {
    store.actions.listDemo.loadInitNews();
    store.event.on('topstories-updated', this.update)
  },

  destroyed () {
    store.event.removeListener('topstories-updated', this.update)
  },

  methods: {
    update () {
      store.actions.listDemo.fetchItemsByPage(this.page).then(items => {
        this.items = items
      })
    }
  },
  ...
  filters: {
    formatItemIndex (index) {
      return (this.page - 1) * store.states.listDemo.data.storiesPerPage + index + 1
    }
  }
}
</script>
